...
1. **`brk_donchian`** — N-day high/low channel breakout (Turtle Rule 1 = 20, Rule 2 = 55), with the `SMA(50)>SMA(300)` trend filter and 2-ATR stop. Cleanest net-new sign vs the existing `brk_*` family. *Prior:* breakout-momentum on JP daily has inverted before (`vol_breakout_confirm`, `lowprice_volspike`) — test the **un-volume-gated** channel first; do NOT add a volume-confirm gate (it inverted). **→ Stage-0 REJECT (2026-06-28):** the bare new-high break is a *loser* — D20/D55 fresh fires sit BELOW the +0.37% baseline (h10 raw +0.17/+0.22, exc −0.20/−0.14), beta-stripped NEGATIVE (MN −0.15/−0.09), negative medians = adverse selection (buying exhausted resistance). Breakout STRENGTH is flat-to-INVERTED (D20 dose monotone worse, Q1 +0.20 → Q4 +0.11; confirms "don't rank by break size"). The `SMA50>SMA300` trend filter is the only glimmer — it flips the *beta-stripped* excess positive (D20 MN exc +0.12 h10/+0.20 h20; D55 +0.22/+0.25) — but fails the binding bar: raw ≤ baseline (no absolute edge for a long book), NOT FY-robust (MN positive ~half the years, negative FY19/20/22/24, leaning on FY2023 + a partial FY2026 n=430), and a negative-median/few-winners distribution = the exact profile that dies at the 6-slot fill-order null (the PEAD-booster lesson). The lift is from a macro-trend/beta gate = the same regime-conditioning `ma_stage`/the N225-trend gate already showed is beta-timing, not separation. Doesn't earn the null budget. See `src/analysis/brk_donchian_stage0.py` and memory `project_brk_donchian_stage0_reject.md`.
...
- **2.5N initial stop & half-rise trail** — A/B the book's 2.0N-3.0N stop band and the "raise stop by ½ the rise" trail against the current `ZsTpSl` / `adx_trail_d8.0` baseline (note `project_confluence_exit_ab_reject` — exit A/Bs have washed out at the portfolio null before). **→ A/B REJECT (2026-06-28), keep ZsTpSl.** Ran the proven `confluence_exit_ab` harness with a `KojiroTrail` arm (k·ATR initial stop + half-rise trail, no TP), k∈{2.5,3.0}. The mechanic works *as the book describes* — Part-1 per-trade is the Turtle profile (DR 37-40% vs ZsTpSl 58.5%, occasionally bigger wins; kj_h3.0 mean_r +2.04% even edges zs +1.87%). But it FAILS the binding paired fill-order null, leaning NEGATIVE: kj_h2.5 Δ Sharpe −0.184 (P0.21, book-Sharpe 0.81 vs zs 1.00), kj_h3.0 −0.136 (P0.24, 0.86). The low-DR/lumpy return shape actively hurts the 6-slot ~36-trade/yr book (can't diversify the added per-trade variance); high-corr per-trade Δ negative (the timestop40 sign-flip tell). The wider 3.0N stop brackets the entry-ATR approximation upward and still rejects. adx_d8 anchor reproduced its known coin-flip (Δ +0.069, P0.60), validating the harness. See `src/analysis/confluence_kojiro_exit_ab.py` and memory `project_kojiro_exit_ab_reject.md`. **5th "let-the-stop-run" exit REJECT at this trade count** (after asym/peak/timestop40/milestone — `project_confluence_exit_ab_reject`).
