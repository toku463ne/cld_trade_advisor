"""SAME-DAY-only confluence vs fill-order null: paired shuffles (read-only).

The single-draw A/B (confluence_sameday_ab.py) showed forcing every sign's
valid_bars to 0 (a fire counts toward the >=N gate ONLY on its fire date, not for
its full validity window) moves the stitched book within noise and FLIPS sign
between the equal-weight book (+0.72 -> +0.88) and the budget book (+0.88 -> +0.83),
and per-FY sign-flips year to year — the fingerprint of fill-order luck.

This is the binding test (cf. confluence_drop_tenkan_null.py). The two arms have
DIFFERENT candidate pools (same 10-sign fires, but SAMEDAY reaches >=3 on far fewer
days), so we pair by SHARED SEED: for each shuffle k, shuffle arm A's pool and arm
B's pool each with rng(k); record stitched Sharpe/return/maxDD per arm; Δ = B − A.

Arm A: WINDOWED (production valid_bars).   Arm B: SAMEDAY (valid_bars=0).  Both 6-slot.

REAL if B's distribution sits above A net of fill-order luck: P(Δ>0) >= 0.95 AND
95% CI of paired Δ excludes 0. Otherwise the single-draw difference is noise.

Run: PYTHONPATH=. uv run --env-file devenv python -m src.analysis.confluence_sameday_null
"""
...
_WINDOWED = dict(_FULL)                 # arm A — production windows
_SAMEDAY = {k: 0 for k in _FULL}        # arm B — fire counts only on fire date
_N_GATE = 3
_K = 200
...
def run() -> None:
    ...
    fires = _fires(_FULL)        # same 10-sign fires feed both arms
    ...
    for cfg in _FYS:
        ...
        pool_a = _pool(_WINDOWED)
        pool_b = _pool(_SAMEDAY)
    ...
    print("\n" + "=" * 82)
    print(f"SAMEDAY vs WINDOWED — FILL-ORDER NULL — {_K} paired shuffles, 6-slot book")
    print("=" * 82)
    print(f"\n{'arm':<22}{'Sharpe mean':>13}{'sd':>7}{'p5':>8}{'p50':>8}{'p95':>8}"
          f"{'ret mean':>10}{'DD mean':>9}")
    for a, lbl in [("A", "WINDOWED (prod)"), ("B", "SAMEDAY (vb=0)")]:
        s_ = sh[a]
    ...
    d = sh["B"] - sh["A"]
    print(f"\n[paired Δ Sharpe = SAMEDAY − WINDOWED, same seed each draw]")
    ...
    ddd = dd["B"] - dd["A"]
    print(f"  paired Δ maxDD mean {ddd.mean()*100:+.1f}pp (positive = SAMEDAY has shallower DD)")

    verdict = ("REAL — SAMEDAY band sits above WINDOWED net of fill-order luck"
               if (d > 0).mean() >= 0.95 and np.percentile(d, 2.5) > 0
               else "NOT separated — single-draw difference is within fill-order noise")
    print(f"\n  VERDICT: {verdict}")
