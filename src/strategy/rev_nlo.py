"""RevNloStrategy — Capitulation bounce after confirmed N225 trough.

Entry conditions (all must hold on the same hourly bar):
  1. RevNloDetector fires at this exact bar (N225 just confirmed a LOW
     and this stock's drawdown was ≥ 50% of N225's over the same window).
  2. Optional regime gate: N225 daily close > 20-day SMA (bull market only).

Exit — first of:
  Time stop : max_hold_days trading days after entry.
  Hard stop : entry_price − atr_stop_mult × ATR14_daily.
...
class RevNloParams:
    ...
    use_regime_gate: bool  = False  # False = skip N225>SMA20 filter
    capital_pct:     float = 0.10   # fraction of equity per trade
...
class RevNloStrategy(Strategy):
    ...
    def __init__(
        ...
        self.params         = params
        self._allowed_dates = allowed_dates
        self._corr_regime   = corr_regime
        bars                = stock_cache.bars
        self._dts           = [b.dt for b in bars]

        # ── Sign detector ─────────────────────────────────────────────────
        self._detector = RevNloDetector(stock_cache, n225_cache)
